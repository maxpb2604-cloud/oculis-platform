    ...
    <div>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3">
        ...
        <span className="text-[12px]" style={{ color: "var(--text-muted)" }}>
          {inChamber.length}{" "}
          {chamber === "SENADO" ? (es ? "senadores" : "senators") : es ? "diputados" : "deputies"}
        </span>
      </div>
        ...
        {tab === "legisladores" && (
          <>
            <Select
              ...
              all={es ? "Todas las provincias" : "All provinces"}
              options={chamberProvinces}
            />
            <Select
              value={party}
              onChange={setParty}
              all={es ? "Todos los partidos" : "All parties"}
              options={chamberParties}
            />
          </>
        )}
      </div>
...
function ProfileModal({
  ...
  const memberType = chamberLabel(l.chamber, es, true);
  // A short, honest bio composed from the structured facts we hold (no invented prose).
  const bioParts = [
    `${memberType}${l.province ? ` ${es ? "por" : "for"} ${l.province}` : ""}`,
  ...
  const initials = l.fullName
    ...
    .join("")
    .toUpperCase();
  const officialUrl = safeHttpUrl(l.sourceUrl);
    ...
    >
      <div>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <h2
                ...
                className="serif text-[19px] font-semibold leading-tight"
              >
                {l.fullName}
              </h2>
              <button
                onClick={onClose}
                className="rounded-md px-2 py-1 text-xs font-semibold hover:bg-[var(--surface-2)]"
            ...
            {l.role && (
              <div className="mt-0.5 text-[12px] font-medium" style={{ color: "var(--accent)" }}>
                {l.role}
              </div>
            )}
        ...
        <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-[12.5px]">
          <Field label={es ? "Cámara" : "Chamber"} value={chamberName(l.chamber, es)} />
        ...
        <div className="mt-4">
          ...
          {l.committees.length === 0 ? (
            <p className="mt-1 text-[12px]" style={{ color: "var(--text-muted)" }}>
              {es ? "Comisiones: No informado." : "Committees: Not reported."}
            </p>
          ) : (
                  ...
                  >
                    ...
                    {c.cargo && c.cargo !== "Miembro" && (
                      <span
                        ...
                        style={{
                          background: CARGO_SOFT[c.cargo] ?? "var(--surface-2)",
                          color: CARGO_COLOR[c.cargo] ?? "var(--text-muted)",
                        }}
                      >
                        {c.cargo}
                      </span>
          ...
          {presidencias.length > 0 && (
            <p className="mt-1.5 text-[11px]" style={{ color: "var(--verified)" }}>
      ...
      >
        <div className="min-w-0">
          ...
          <div className="mt-0.5 text-[11px]" style={{ color: "var(--text-muted)" }}>
            ...
            {president ? ` · ${es ? "Pres." : "Chair"}: ${president.name}` : ""}
          </div>
        </div>
        <span className="shrink-0 text-[11px] font-semibold" style={{ color: "var(--text-muted)" }}>
          {open ? (es ? "Ocultar" : "Hide") : es ? "Mostrar" : "Show"}
        </span>
      </button>
      {open && (
