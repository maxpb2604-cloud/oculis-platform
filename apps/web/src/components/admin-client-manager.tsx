...
export function AdminClientManager({
  ...
  async function changePassword(userId: number, form: FormData) {
    ...
    if (password !== confirmation) {
      setMessage({
        tone: "error",
        text: es ? "Las contraseñas no coinciden." : "Passwords do not match.",
      });
      return;
    }
    setBusy(true);
    ...
    try {
      ...
      if (
        adminUsers.some(
          (user) => user.id === userId && user.email.toLowerCase() === adminEmail.toLowerCase(),
        )
      ) {
        await signOut();
        return;
      ...
      setMessage({
        tone: "ok",
        text: es
          ? "Contraseña cambiada. Las sesiones anteriores de esa cuenta quedaron invalidadas."
          : "Password changed. Existing sessions for that account are now invalid.",
      });
      router.refresh();
    } catch (error) {
      setMessage({ tone: "error", text: (error as Error).message });
    ...
    return (
      ...
      >
        ...
        <p className="text-xs text-[var(--text-muted)] sm:col-span-2">
          {es
            ? "Mínimo 12 caracteres. La contraseña actual no puede mostrarse; al guardar, se reemplaza y se cierran las sesiones anteriores."
            : "Minimum 12 characters. The current password cannot be shown; saving replaces it and invalidates existing sessions."}
        </p>
        <button
          disabled={busy}
          className="min-h-10 rounded-lg bg-[var(--accent)] px-4 text-sm font-semibold text-white disabled:opacity-50 sm:col-span-2"
        >
          {busy
            ? es
              ? "Guardando…"
  ...
  async function signOut() {
    ...
    window.location.assign(`/${lang === "en" ? "?lang=en" : ""}#acceso`);
  }
    ...
    <div data-testid="admin-client-manager">
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex shrink-0 flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={signOut}
              className="ui-button min-h-11 gap-2 px-4 text-sm"
            >
        ...
        <p className="mt-4 text-xs text-[var(--text-muted)]">
          {es
            ? `Sesión iniciada como ${adminName} (${adminEmail}).`
        ...
        {addingAdmin ? (
          <form
            action={addAdmin}
            className="mt-5 grid gap-3 rounded-lg border bg-[var(--surface-2)] p-4 sm:grid-cols-2"
          >
            <div>
            ...
            <button
              disabled={busy}
              className="min-h-11 rounded-lg bg-[var(--accent)] px-4 text-sm font-semibold text-white disabled:opacity-50 sm:col-span-2"
            >
              {busy
                ? es
                  ? "Guardando…"
      ...
      <section
        aria-label={es ? "Resumen de clientes" : "Client overview"}
        className="mt-8 grid gap-4 border-b pb-7 sm:grid-cols-3"
      >
        <AdminStat
      ...
      <section aria-labelledby="client-access-title" className="mt-7">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 id="client-access-title" className="section-title mt-2">
              {es ? "Accesos de clientes" : "Client access"}
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-[var(--text-muted)]">
              {es
                ...
                <section className="px-5 py-5" aria-labelledby={`client-${client.id}-users`}>
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setAddingUserFor(addingUserFor === client.id ? null : client.id)
                      }
