/**
 * Lightweight layout primitives shared by every page. They live apart from
 * dashboard.tsx on purpose: dashboard.tsx statically imports the chart and map
 * libraries, so pages that only needed a Panel were shipping mapbox-gl +
 * recharts in their First Load JS (~720 kB). Import these from here instead.
 */
...
export function SectionHeading({ n, title }: { n?: string; title: string }) {
  return (
    <div className="mb-3 mt-8 flex items-baseline gap-3">
      {n && <span className="tnum text-xs font-semibold" style={{ color: "var(--accent)" }}>{n}</span>}
