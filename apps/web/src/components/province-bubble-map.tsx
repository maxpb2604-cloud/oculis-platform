      ...
      {legs.length === 0 ? (
        ...
        <ul className="mt-1.5 space-y-1.5">
          {legs.map((l) => {
            ...
            return (
              <li key={l.profileId} className="leading-tight">
                <LegislatorProfileTrigger
                  ...
                  ariaLabel={
                    lang === "es"
                      ? `Abrir perfil de ${l.fullName}, ${party}`
                      : `Open ${l.fullName}'s profile, ${party}`
                  }
                  className="-ml-2 inline-flex min-h-11 max-w-full items-center rounded-md px-2 text-left text-[12.5px] font-medium underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
                >
                  {l.fullName}
                </LegislatorProfileTrigger>
                <div className="flex items-start gap-1.5 text-[10.5px] text-white/50">
                  <span
                    aria-hidden="true"
                    className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full"
                    style={{ background: partyColor(l.party) }}
                  />
                  <span>{[l.role, party].filter(Boolean).join(" · ")}</span>
                </div>
              </li>
            );
          })}
