...
import { createDb, initiatives, scoreInputs, statusEvents } from "@oculis/db";
...
const scores = await src.db.select().from(scoreInputs);
const events = await src.db.select().from(statusEvents);
await src.close();
console.log(`read from PGlite: ${inits.length} initiatives, ${scores.length} score_inputs, ${events.length} events`);
...
for (const s of scores) {
  const newId = idMap.get(s.initiativeId);
  if (!newId) continue;
  const { initiativeId, ...rest } = s;
  await dest.db.insert(scoreInputs).values({ ...rest, initiativeId: newId }).onConflictDoNothing();
}
for (const e of events) {
  const newId = idMap.get(e.initiativeId);
  if (!newId) continue;
  const { id, initiativeId, ...rest } = e;
  await dest.db.insert(statusEvents).values({ ...rest, initiativeId: newId }).onConflictDoNothing();
}
await dest.close();
